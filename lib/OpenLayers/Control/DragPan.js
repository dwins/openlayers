...
OpenLayers.Control.DragPan = OpenLayers.Class(OpenLayers.Control, {
    ...
    */
     ...
     * xy - {<OpenLayers.Pixel>} Pixel of the mouse position
     */
    panMapDone: function(xy, type) {
        if(this.panned) {
            var res = null;
            if (this.kinetic) {
                res = this.kinetic.end(xy);
            }
            var dragging = type === "mouseout" || !!res;
            this.map.pan(
                this.handler.last.x - xy.x,
                this.handler.last.y - xy.y,
                {dragging: dragging, animate: false}
            );
            if (dragging && xy.x | 0 && xy.y | 0) {
                this.map.pan(0, 0, {dragging: false, animate: false});
            }
            if (res) {
                var self = this;
